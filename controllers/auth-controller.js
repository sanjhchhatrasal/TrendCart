...
module.exports.cartPage = async (req, res) => {
  try {
    ...
    const bill = Number(product.price) + 20 - Number(product.discountedPrice);

    console.log(product.price, product.discountedPrice);
...
module.exports.profilePic =  async (req, res) => {
    try {
        ...
        await userModel.findByIdAndUpdate(req.user.id, { profilePic: req.file.buffer });
